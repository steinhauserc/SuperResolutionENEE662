...
function [ A, b, G ] = formulateProblemCOPY(images, offsets, scaleFactor, blurSigma)
    A = [];
    b = [];
    superSize = scaleFactor * size(images{1}) + [ 1 1 ];
    for i = 1 : numel(images)
        transMat = TransMat(superSize, offsets(i, :));
        blurMat = BlurMat(superSize, blurSigma);
        decMat = DecMat(superSize, scaleFactor);
    ...
    G = [convmtx2([1 -1], superSize);
         convmtx2([1; -1], superSize);
         convmtx2([1 0; 0 -1], superSize);
         convmtx2([0 1; -1 0], superSize)];
end
...
function decMat = DecMat(superSize, scaleFactor)
    sampledSize = 1 / scaleFactor * (superSize - 1);
    [ outputRow outputCol ] = meshgrid(1 : sampledSize(1), 1 : sampledSize(2));
    inputRow = scaleFactor * outputRow;
    inputCol = scaleFactor * outputCol;
